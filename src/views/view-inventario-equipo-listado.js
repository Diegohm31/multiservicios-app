    ...
    render() {
      ...
      <div class="grid">
        ${this.equipos.map(equipo => html`
          <div class="card">
            <div class="card-body">
              <h2 class="card-title">${equipo.modelo}</h2>
              <div class="card-nivel">${equipo.descripcion}</div>
              <div class="card-tarifa">${equipo.codigo_interno}</div>
              <div class="card-disponible">${equipo.disponible ? 'Disponible' : 'No disponible'}</div>
              <div class="card-fecha_adquisicion">${equipo.fecha_adquisicion}</div>
              <!-- acciones para editar y eliminar -->
              <div class="actions">
                <button class="btn btn-edit" @click=${(e) => { e.preventDefault(); navigator.goto(`/inventario/edit/equipo/${equipo.id_equipo}`); }}>Editar</button>
                <button class="btn btn-delete" @click=${(e) => { e.preventDefault(); this.deleteEquipo(equipo.id_equipo); }}>Eliminar</button>
              </div>
            </div>
          </div>
        `)}
